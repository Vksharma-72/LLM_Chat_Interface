"""Chat & conversations test fixtures (§12).

Same isolation scheme as tests/auth: tables truncated + Redis db 1 flushed per
test (the app commits per request). The LLM service is a fresh per-test
instance injected via dependency override (the cached singleton would hold
event-loop-bound Redis connections across tests). Upstream HTTP is respx-mocked.
"""
...
async def chat_llm(app, monkeypatch):
    """Fresh LLMService per test, injected over the cached singleton."""
...
@pytest.fixture
def parse_sse():
    """Collect (event_name, data_dict) pairs from a streaming SSE response."""
