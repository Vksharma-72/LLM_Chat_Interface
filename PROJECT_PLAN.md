...
| 5 — Conversations + chat API | done | 2026-09-06 | conversations CRUD+search (q over titles+message content, pinned first, pagination, owner-scoped 404); chat send + SSE stream (meta→delta*→done, auto-title, usage increments, user message kept on 502 llm_unavailable); chat models/status (Redis-cached); slowapi+Redis limits (chat 20/min/user, API 120/hour/user, Retry-After). Gate green: FULL pytest 102 passed + 1 skipped, curl demo of send+stream vs mock LLM, ruff clean |
| 6 — Frontend scaffold + auth | done | 2026-09-06 | Vite+React-TS+Tailwind (class dark mode) with all §2 deps; authStore (zustand, persisted) + api.ts (401→refresh-once→retry, logout on failure); login/register pages with validation+error banners, ProtectedRoute, ThemeToggle; backend mounts frontend/dist at / with SPA fallback (single origin :3001). Gate green: npm run build, vitest 12 passed, Playwright auth spec 3 passed (chromium), backend FULL pytest 102 passed + 1 skipped, ruff clean |
...
| 10 — Attachments backend | done | 2026-09-06 | attachments table (migration 64ebe5a0419b both DBs), streaming upload + MIME/magic detection, Pillow derivative, pypdf/docx/text extraction, owner-scoped content serving, path-aware body cap, multimodal prompt builder + vision fallback, orphan cleanup, mock multimodal. Gate green: FULL pytest 137 passed + 1 skipped, ruff clean, curl demo (image -> mock vision reply, doc -> extraction in prompt) |
| 11 — Attachments UI | done | 2026-09-06 | 📎 multi-file upload (drag-drop, clipboard paste, progress chips, size pre-check), attachment_ids in send body, optimistic temp message with attachments, image thumbnails + Lightbox (Esc/arrows), inline video/audio, document download chips, friendlyError entries. Gate green: backend FULL pytest 137 passed + 1 skipped, vitest 30 passed, Playwright 11 passed, build OK, ruff clean, smoke.sh 10/10 |
